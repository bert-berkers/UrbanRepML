#!/usr/bin/env python3
"""
Process AlphaEarth 2023 TIFFs for Pearl River Delta to H3 resolution 10.

High-resolution processing with 3M+ hexagons for detailed urban analysis.
...
def setup_logging():
    """Setup detailed logging for Pearl River Delta resolution 10 processing."""
    ...
    log_file = log_dir / f'pearl_river_delta_res10_2023_{timestamp}.log'
...
def process_alphaearth_prd(logger):
    ...
    logger.info("="*60)
    logger.info("STARTING PEARL RIVER DELTA ALPHAEARTH PROCESSING (RESOLUTION 10)")
    logger.info("="*60)

    start_time = time.time()

    # Configuration for Pearl River Delta at resolution 10
    config = {
        'source_dir': 'G:/My Drive/AlphaEarth_PRD/',  # Direct path to PRD data
        'subtile_size': 256,  # Smaller tiles for resolution 10 (vs 512 for res 8)
        'min_pixels_per_hex': 3,  # Lower threshold for smaller hexagons
        'max_workers': 10,  # Parallel processing
        'intermediate_dir': 'intermediate_prd_res10'  # Separate intermediate embeddings stage1_modalities directory
    }
    ...
    logger.info(f"  Expected output: ~3M hexagons (49x more than res 8)")
...
def save_processed_data(gdf, processing_duration, logger):
    ...
    output_file = output_dir / 'prd_res10_2023.parquet'
    ...
    metadata = {
        ...
        'coverage_area_km2': len(gdf) * 0.015,  # Approximate area for res 10
        ...
        'notes': [
            ...
            'High resolution processing: ~49x more hexagons than resolution 8'
        ]
    }
    ...
    # Save metadata
    metadata_file = output_dir / 'prd_res10_2023_metadata.json'
...
def main():
    ...
    try:
        ...
        # Log session start
        logger.info("PEARL RIVER DELTA ALPHAEARTH PROCESSING SESSION (RESOLUTION 10)")
        ...
        logger.info("="*60)
        logger.info(f"Study area: Pearl River Delta (Resolution 10)")
