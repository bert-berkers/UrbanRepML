#!/usr/bin/env python3
"""
Fast Resolution 10 Clustering Visualization Script using Datashader and Dissolve
...
def load_and_prepare_embeddings(data_path: Path) -> Tuple[gpd.GeoDataFrame, np.ndarray]:
    ...
    try:
        ...
        # Create H3 hexagon geometries using SRAI if h3_index is available
        if 'h3_index' in df.columns:
            # Use SRAI's h3_to_geoseries for proper H3 hexagon geometries
            ...
            raise ValueError("No geometry, h3_index, or lat/lng columns found in data")
...
def add_coordinate_grid_and_labels(ax, bounds, target_crs: str):
    ...
    if target_crs == 'EPSG:3857':  # Web Mercator
        ...
        # Create grid
        lon_step = 0.5  # Coarser grid for speed
        lat_step = 0.5
...
def create_fast_cluster_visualization(gdf: gpd.GeoDataFrame, clusters: np.ndarray,
    ...
    # Create figure
    fig, ax = plt.subplots(figsize=(12, 12))  # Slightly smaller for speed
    ...
    # Extract resolution from filename if it contains 'res' pattern
    import re
    res_match = re.search(r'res(\d+)', str(data_to_plot.crs) if hasattr(data_to_plot, 'crs') else '')
    res = 8 if study_area == 'cascadia' else 10  # Cascadia is res8, others res10
...
def main():
    ...
    # Parse arguments
    parser = argparse.ArgumentParser(description='Fast Resolution 10 Clustering Visualization')
    ...
    print("="*80)
    print("FAST RESOLUTION 10 CLUSTERING VISUALIZATION")
    print("="*80)
    ...
    for n_clusters, colormap in COLORMAP_COMBINATIONS:
        if n_clusters in cluster_results:
            ...
            filename = f'{args.study_area}_res10_{n_clusters:02d}clusters_{colormap}_{suffix}.png'
