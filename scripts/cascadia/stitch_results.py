...
def load_and_combine_intermediates(json_files: List[Path], config: dict, logger) -> pd.DataFrame:
    ...
    # Apply targeted gap elimination to fix tile boundary issues
    logger.info("Applying targeted gap elimination for tile boundary fixes...")
    try:
        df_processed, quality_metrics = apply_targeted_gap_elimination(df, config)
        logger.info("Gap elimination completed successfully")
        logger.info(f"Processed {quality_metrics['boundary_hexagons']} boundary hexagons")
...
Examples:
  ...
  python stitch_results.py --intermediate-dir custom/    # Custom intermediate directory
    ...
    try:
        ...
        df = load_and_combine_intermediates(json_files, config, logger)
