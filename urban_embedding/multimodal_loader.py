...
class MultiModalLoader:
    ...
    def load_modality(self, name: str, config: Dict) -> pd.DataFrame:
        ...
        # Apply preprocessing auxiliary data if specified
        if 'preprocessing auxiliary data' in config:
            df = self._preprocess_modality(df, config['preprocessing auxiliary data'], name)
            
        return df
    
    def _preprocess_modality(self, df: pd.DataFrame, preprocess_config: Dict, 
                            name: str) -> pd.DataFrame:
        """Apply preprocessing auxiliary data to a modality."""
    ...
    def load_all_modalities(self) -> Dict[str, pd.DataFrame]:
        """Load all enabled modalities from configuration."""
        logger.info("Loading all modalities...")
        
        modality_configs = self.config.get('modalities', {})
        ...
        logger.info(f"Loaded {len(self.modalities)} modalities: {list(self.modalities.keys())}")
        return self.modalities
    
    def align_modalities(self, method: str = 'intersection') -> pd.DataFrame:
        """Align modalities to common H3 cells."""
        if not self.modalities:
            raise ValueError("No modalities loaded")
            
        logger.info(f"Aligning {len(self.modalities)} modalities using {method} method")
        
        # Get all H3 indices
        h3_sets = {name: set(df.index) for name, df in self.modalities.items()}
        
        if method == 'intersection':
            # Only keep H3 cells present in all modalities
        ...
        # Align all modalities to common index
        aligned = {}
    ...
    def fuse_modalities(self, method: str = 'concatenate', 
        ...
        """Fuse aligned modalities into single feature matrix."""
        ...
        logger.info(f"Fusing modalities using {fusion_method} method")
        ...
        if fusion_method == 'concatenate':
            ...
            if weights is None:
                weights = {name: self.config['modalities'][name].get('weight', 1.0) 
...
def load_multimodal_embeddings(config_path: str, 
                              ...
                              output_dir: str = 'data/processed/multimodal') -> pd.DataFrame:
