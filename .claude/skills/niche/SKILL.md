...
4.5. **Kapstok discovery** (fallback when step 4 didn't surface a plan): glob `.claude/plans/{today}-*.md`. Kapstoks are written by `/valuate` Step 5.6 to scaffold the upcoming `/niche` invocation; they encode the active supra state.
   ...
   - **Multiple matches**: prefer the one whose intent-slug matches the active supra intent (compute via `slugify(supra_intent[:80])` from `.claude/skills/valuate/plan_kapstok.py`). If still ambiguous, ask the user "I found {N} kapstoks for today: [list]. Which is primary?". Do not silently pick.
